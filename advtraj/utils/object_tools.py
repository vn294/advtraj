            ...
            if obj_i_on_border:
                ...
                while j < nobjs:
                    ...
                    if obj_j_on_opposite_border:
                        ...
                        if debug_label:
                            print(
                                f"Match Object {j:03d} on {['x','y'][dim]}={border} border?"
                            )
                        ...
                        # x or y intersection
                        int1 = np.intersect1d(
                            posi[1 - dim][ilist], posj[1 - dim][jlist]
                        )
                        # z-intersection
                        int2 = np.intersect1d(posi[2][ilist], posj[2][jlist])
...
def get_object_labels(mask: xr.DataArray) -> xr.DataArray:
    ...
    olab = (
        olab.where(mask, drop=True)
        .stack(trajectory_number=("x", "y", "z"))
        .dropna(dim="trajectory_number")
    )

    olab = olab.assign_coords(
        trajectory_number=np.arange(olab.trajectory_number.size)
    ).astype(int)
...
def get_bounding_boxes(ds_traj, use_mask=False):
    ...
    if use_mask:
        ds = (
            ds_traj[["x", "y", "z", "object_label", "ref_time"]]
            .where(ds_traj["object_mask"])
            .groupby("object_label")
        )
    else:
        ds = ds_traj[["x", "y", "z", "object_label", "ref_time"]].groupby(
            "object_label"
        )

    ds_num = ds.count(dim="trajectory_number")
    ds_num_in_obj = ds_num.x
    ...
    ds_count = ds_num_in_obj.rename("num_in_obj")
    ds_out = xr.merge([ds_min, ds_max, ds_mean, ds_count])

    return ds_out


def get_field_avg(ds_traj, use_mask=False):
    """
    Find x,y,z min and max for objects in ds_traj.
    ...
    if use_mask:
        ds = (
            ds_traj[["l", "object_label"]]
            .where(ds_traj["obj_mask"])
            .groupby("object_label")
        )
    else:
        ds = ds_traj[["l", "object_label"]].groupby("object_label")
